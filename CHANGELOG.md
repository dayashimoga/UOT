...
- **Rust Coverage CI Validation (`cargo tarpaulin`)**:
  - Added `mkdir -p rust/target/coverage` before `cargo tarpaulin` execution in `.github/workflows/ci.yml` to resolve `tee: rust/target/coverage/tarpaulin.log: No such file or directory`.
  - Added `--exclude-files "*load_stress*"` to `cargo tarpaulin` command line to skip 100MB+ load stress test ptrace instrumentation overhead during coverage calculation, preventing CI job timeout/cancellation while maintaining 100% test execution in `cargo test`.
